...
export const MealItem = ({ meal, i, middleIdx, setMealIdx, isSameDay, checkShowMealDate }: MealItemProps) => (
  <li
  ...
  >
    <div>
      {((isSameDay && middleIdx === i) || (!isSameDay && checkShowMealDate(meal, i))) && (
        <h3 className={styles.date}>{dateToKoreanDateStr(shrotDateStrToDate(meal.date))}</h3>
      )}
      {meal.time && (
        <h4 className={`${styles.meal_info} ${calColor(meal.cal ?? 0)}`}>
