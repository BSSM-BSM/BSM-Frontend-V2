			...
			<div className={styles.post_wrap}>
				<div className={styles.post_info}>
					<div className={`user-profile ${styles.user_profile}`}>
						<Image src={profileSrc} onError={() => setProfileSrc(DefaultProfilePic)} width="128" height="128" alt="user profile" />
					</div>
					<div className="cols space-between flex-main">
						...
						<div className="rows space-between">
							...
							<div className="rows gap-2 gray">
								...
								<span>{boardDetailTime ? new Date(post.createdAt).toLocaleString() : elapsedTime(post.createdAt)}</span>
								{post.permission && <DropdownMenu meatballsMenu={true} menus={dropdownMenu} />}
							</div>
						</div>
					</div>
				</div>
				<div className={styles.post_content} dangerouslySetInnerHTML={{ __html: postXssFilter.process(post.content) }} />
			</div>
			<div className={styles.like_wrap}>
				<button
					onClick={() => postLike(post.myLike === 1 ? 'NONE' : 'LIKE')}
					className={`button-wrap ${styles.like} ${post.myLike === 1 ? styles.on : ''}`}>
					<img src="/icons/like.svg" alt="like" />
				</button>
				<span>{post.totalLikes}</span>
				<button
					onClick={() => postLike(post.myLike === -1 ? 'NONE' : 'DISLIKE')}
					className={`button-wrap ${styles.dislike} ${post.myLike === -1 ? styles.on : ''}`}>
